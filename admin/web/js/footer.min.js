...
$(document).on("click", "button.modPwd_button", function () {
	var b = $("#modPwd_curPwd").val();
	var a = $("#modPwd_newPwd").val();
	var c = $("#modPwd_newPwd2").val();
	if (!b.length) {
	...
	$.post("/api/user", {tag: "pwd", curPwd: b, newPwd: a}, function (d) {
		if (d.code == 0) {
			layer.closeAll();
			layer.msg(d.msg);
			setTimeout(function () {
			...
			layer.msg(gIconAlert + d.msg)
		}
	}, "json")
});
$(document).on("click", "button.modProfile_btn", function () {
	var f = $("#modProfile_name");
	var e = $("#modProfile_fullname");
	var d = $("#modProfile_phone");
	var c = $.trim(f.val());
	var b = $.trim(e.val());
	var a = $.trim(d.val());
	if (!c.length) {
		layer.msg(gIconAlert + f.attr("placeholder"));
		return
	} else {
		if (!b.length) {
			layer.msg(gIconAlert + e.attr("placeholder"));
			return
		} else {
			if (!a.length) {
				layer.msg(gIconAlert + d.attr("placeholder"));
				return
			}
		}
	}
	$.post("/api/system/branch", {tag: "modify", name: c, fullname: b, phone: a}, function (g) {
		if (g.code == 0) {
			layer.closeAll();
			layer.msg(gIconOK + g.msg);
			setTimeout(function () {
				location.reload()
			}, 600)
		} else {
...
function resetLeftMenuScroll() {
	...
	var f = 0;
	var e = $("#side-menu");
	var a = e.find("a.nav-top-menu");
	var d = a.get(0).offsetHeight;
	f += d * a.length;
	var c = 10;
	$.each(e.find("ul.nav-second-level"), function () {
	...
	var g = 32;
	if (e.find("a.cur-sub-nav").length) {
		g = e.find("a.cur-sub-nav").get(0).offsetHeight
	}
	var b = (f + g * c);
	console.log(b);
	$("#treeScroller").css({height: b});
	new IScroll("#nav-left-menus", {mouseWheel: true, click: false})
}
function rollNumbers() {
	var a = {useEasing: false, useGrouping: false, separator: "", decimal: ".", prefix: "", suffix: ""};
	$.each($(".large[data-val]"), function () {
		var c = $(this);
		var b = c.attr("data-val");
		var d = new CountUp(c.get(0), 0, b, 0, 1.5, a);
		d.start()
	})
}
...
function checkNotice() {
	...
	$.post("/api/admin/notice", {tag: "check", id: mAdminInfoId}, function (a) {
		if (a.code == 0) {
			console.log("checkNotice -- ");
			if (a.data.todo && mAdminTODO.length) {
				mAdminTODO.html(Mustache.render(mAdminTODO_tmp, {items: a.data.todo}))
			}
			if (a.data.wxmsg && mAdminWXMSG.length) {
				mAdminWXMSG.html(Mustache.render(mAdminWXMSG_tmp, {items: a.data.wxmsg}));
				if (!!a.data.wxmsg_unread) {
...
function bpbPopupImage(d, a) {
	var c = '<div class="w-show"><img class="w-image-big" src="{[src]}" alt="">{[#title]}<p class="title">{[title]}</p>{[/title]}</div>';
	var e = {src: d, title: ""};
	if (a) {
		e.title = a
	}
	var b = Mustache.render(c, e);
	layer.open({type: 1, title: false, area: ["400px", "400px"], shadeClose: 1, content: b})
}
