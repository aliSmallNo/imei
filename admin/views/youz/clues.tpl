...
<div class="row">
	<form method="get" class="form-inline" action="/youz/clues">
	...
	<ul class="nav nav-tabs">
		{{foreach from=$tabs key=key item=tab}}
			<li class="ng-scope {{if $cat== $key}} active{{/if}}">
				<a href="/youz/clues?cat={{$key}}&sort={{$sort}}&{{$urlParams}}"
		...
		{{foreach from=$items item=prod}}
			<tr>
				...
				<td class="cell-act" data-id="{{$prod.cId}}">
					...
					{{/if}}
					{{if $cat=="my"}}
						<a href="javascript:;" class="btnChange btn btn-outline btn-info btn-xs" style="display: none">转给他人</a>
					{{/if}}
				</td>
			</tr>
		{{/foreach}}
	...
	$(document).on('click', '#btnSaveMod', function () {
		...
		switch (tag) {
			...
			case "yxs_clue_edit":
				...
				if (!postData["name"] || !postData["wechat"] || !postData['phone'] || !postData['bd']) {
					layer.msg("严选师姓名、微信号、手机号、bd不能为空！");
					return;
				}
				url = '/api/youz';
				break;
