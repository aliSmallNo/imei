...
<div class="row">
  <form action="/stock/stock_main2" method="get" class="form-inline">
  ...
  <table class="table table-striped table-bordered">
    ...
    {{foreach from=$list item=item key=key}}
      <tr>
        ...
        <td class="border_r">{{$item.m_cus_rate2}}%</td>

        <!-- 上证涨跌 -->
        <td>{{$item.s_sh_change}}%</td>
        <!-- 散户比值 -->
        <td>
          <div>均值 {{$item.s_cus_rate_avg2}}%</div>
          <div>均值比例 {{$item.s_cus_rate_avg_scale2}}%</div>
        </td>
        <!-- 合计交易额 -->
        <td>
