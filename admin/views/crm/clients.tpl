		...
		<tr>
			...
			<th class="col-lg-4">
				...
				<a {{if $sort=='dd' || $sort=='da'}}class="active"{{/if}}
					 href="/crm/clients?cat={{$cat}}&sort={{$dNext}}&{{$urlParams}}">跟进日期 <i class="fa {{$dIcon}}"></i></a>
				<a {{if $sort=='sd' || $sort=='sa'}}class="active"{{/if}}
					 href="/crm/clients?cat={{$cat}}&sort={{$sNext}}&{{$urlParams}}">跟进进度 <i class="fa {{$sIcon}}"></i></a>
			</th>
			<th>
				操作
			</th>
		...
		{{foreach from=$items item=prod}}
			<tr>
				...
				<td class="cell-act" data-id="{{$prod.cId}}">
					<a href="/crm/detail?id={{$prod.cId}}" class="btnDetail btn btn-outline btn-primary btn-xs">跟进详情</a>
