...
<div class="row">
    <div class="col-lg-5 form-horizontal">
            ...
            <div class="col-sm-8">
                <select name="level" required="required" class="form-control">
                    ...
                    {{foreach from=$levels key=k item=level}}
                        <option value="{{$k}}"
                                {{if $userInfo && $userInfo['aLevel']==$k}}selected{{/if}}>{{$level}}</option>
                    {{/foreach}}
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            <label class="col-sm-4 control-label">是否是股票语音电话合作客户:</label>
            <div class="col-sm-8">
                <select name="aIsVoiceParther" required="required" class="form-control">
                    <option>请选择</option>
                    <option value="0" {{if $userInfo && $userInfo['aIsVoiceParther']==0}}selected{{/if}}>否</option>
                    <option value="1" {{if $userInfo && $userInfo['aIsVoiceParther']==1}}selected{{/if}}>是</option>
                </select>
            </div>
        </div>
...
<script>
  $(".opSave").on("click", function() {
    ...
    $.post("/api/user", {
      ...
      aIsVoiceParther: $("select[name=aIsVoiceParther]").val(),
