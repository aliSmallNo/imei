    ...
    public function statTrend($step, $queryDate, $resetFlag = false)
    {
        ...
        if ($res) {
            ...
            foreach ($res as $field => $num) {
                ...
                $trend['loan_sum_' . $field] = intval($num);
                $trend['loan_sum_total'] += intval($num);
            }
        }
...
//		}
        ...
        $sql = "select 
				...
				count(DISTINCT case when tAddedBy='1069' then tCId end) as lishiqi
...
//		}
        ...
        if ($step == "month") {
            ...
            /**
             * 帮我统计下流失用户
             * 1.按月统计，流失用户（之前交易，这月不交易了）
             * 2.显示出数字即可
             * 3.按照BD名字来统计
             * 4.用曲线图来显示，类似于“统计新用户”（如下图）
             */
            $lose_curr_month_str = "";
            foreach ($salers as $v) {
                $name = Pinyin::encode($v['uName'], 'all');
                $name = str_replace(" ", '', ucwords($name));
                $lose_curr_month_str .= "count(DISTINCT case when uPtPhone='" . $v['uPhone'] . "' then oPhone end) as " . $name . '_' . $v['uPhone'] . ',';
            }
            $lose_curr_month_str = trim($lose_curr_month_str, ',');
            $sql = "select 
                    count(DISTINCT case when uPtPhone then oPhone end) as total,
                    $lose_curr_month_str
                    from im_stock_order as o
                    left join im_stock_user as u on o.oPhone=u.uPhone 
                    where 
