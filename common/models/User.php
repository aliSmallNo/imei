...
//				AND a.aCategory in (1000,1002,1004) ";
	...
	public static function rankCalNew()
	{
		...
		$ret = $conn->createCommand($sql)->queryAll();
		foreach ($ret as $v) {
			self::calOneRank($conn, $v);
		}
	}

	public static function calOneRank($conn, $item)
	{
		...
		$sql = "SELECT count(*) as co from im_chat_msg where cAddedBy=:uid and cAddedOn BETWEEN :sd AND :ed ";
		$link2 = $conn->createCommand($sql);

		$sql = "SELECT SUM(tAmt) as amt from im_user_trans where tUId=:uid and tCategory=:cat and tAddedOn  BETWEEN :sd AND :ed  ";
		$link3 = $conn->createCommand($sql);

		$sql = "select Count(*) as co from im_user_net where  nSubUId=:uid and nRelation=:rel AND  nAddedOn  BETWEEN :sd AND :ed ";
		...
		do {
			...
			if (date("Y-m-d", $dt) == date("Y-m-d", time())) {
				// 记录今天的分数
				User::edit($uid, ["uRankTmp" => $rankVal]);
			}
			$dt += 86400;
		} while ($dt < $time);
