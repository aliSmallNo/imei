...
class StockMainStat extends \yii\db\ActiveRecord
{
    ...
    public static function items($criteria, $params, $page, $pageSize = 20)
    {
        ...
        foreach ($res as $k => $v) {
            ...
            $buy_name = [];
            foreach ($buys as $buy) {
                $flag1 = floatval($buy['r_stocks_gt']) ? $J_s_sh_change > $buy['r_stocks_gt'] : true;
                $flag2 = floatval($buy['r_stocks_lt']) ? $J_s_sh_change < $buy['r_stocks_lt'] : true;
                $flag3 = floatval($buy['r_cus_gt']) ? $L_s_cus_rate_avg_scale > $buy['r_cus_gt'] : true;
            ...
            foreach ($solds as $sold) {
                $flag1 = floatval($sold['r_stocks_gt']) ? $J_s_sh_change > $sold['r_stocks_gt'] : true;
                $flag2 = floatval($sold['r_stocks_lt']) ? $J_s_sh_change < $sold['r_stocks_lt'] : true;
                $flag3 = floatval($sold['r_cus_gt']) ? $L_s_cus_rate_avg_scale > $sold['r_cus_gt'] : true;
