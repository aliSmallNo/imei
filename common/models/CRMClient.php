...
//['#88AACC', '#8A89A6', '#7B6888', '#6B486B', '#A05D56', '#D0743C', '#FF8800']
	...
	{
		$sql = "select c.cId,c.cName,c.cPhone,IFNULL(a.aName,'') as nickname from im_crm_client as c
		...
		if ($ret) {
			...
			$sql = "update im_crm_client set cBDAssign=:aid,cBDAssignDate=now() WHERE cId=:id";
	...
	public static function addFromUser($uid, $adminId = 0)
	{
		...
		$sql = "select u.uId,u.uPhone,u.uName, u.uShopAddress, IFNULL(c.cId,0) as cid 
			from im_user as u 
 			left join im_crm_client as c on u.uPhone = c.cPhone
 			where u.uId=$uid ";
	...
	{
		$conn = AppUtil::db();
		$sql = "delete from im_crm_client";
		$conn->createCommand($sql)->execute();
		$sql = "delete from im_crm_track";
		$conn->createCommand($sql)->execute();

		$sql = "insert into im_crm_client(cName,cPhone,cWechat,cEmail,cProvince,cCity,cIntro,cSource,cAddedDate,cUpdatedDate,cNote,cBDAssignDate,cBDAssign)
		...
		$sql = "insert into im_crm_track(tCId, tNote,tStatus,tDate,tAddedDate,tAddedBy)
  			SELECT cId,(case when cAddedBy>0 then '添加新的客户线索' else '客户在官网上填写的信息' END),100,
   	    	cAddedDate, cAddedDate, cAddedBy from im_crm_client";
		$conn->createCommand($sql)->execute();
		...
		$conn->createCommand("update im_crm_client set cEmail=''")->execute();
	}
	...
	public static function counts($aid, $criteria, $params = [])
	{
		...
		count(1) as cnt
 		from im_crm_client where cDeletedFlag=0 $strCriteria";
		$conn = AppUtil::db();
		$params[":aid"] = $aid;
	...
	public static function funnelStat($category, $beginDate, $endDate, $id = "", $conn = "")
	{
		...
		$sql = "select count(1) as cnt,cStatus from im_crm_client as c
	...
	public static function getList($condition, $page, $pagesize)
	{
		...
		$conn = AppUtil::db();
		$ret = $conn->createCommand($sql)->queryAll();
		$sql = "select count(*) as co from im_crm_client where cCategory=110 and cDeletedFlag=0 $condition";
		$count = 0;
