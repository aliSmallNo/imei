...
//	const URL_GET_IPS = 'http://piping.mogumiao.com/proxy/api/get_ip_bs?appKey=405b848e01284a42a1b2152b48973894&count=10&expiryDate=0&format=1&newLine=2';
	...
	public static function phone_section_2()
	{
		...
		$phone_section = [
			// 1852225,1331203, 1320758, 1355244,1781023, 1862221, 1551095, 1382122, 1586609, 1372238, 1382129, 1892027,1860229, 1862271, 1382127,1382062, 1522226,1592202, 1872221, 1382031,
			1302135, 1362218, 1502258, 1502239, 1392041, 1375220, 1361208, 1392069, 1351622, 1860226, 1512216, 1863099, 1552293, 1852236, 1592208,
		];
		foreach ($phone_section as $p) {
			self::combind_phone($p);
		}
	...
	{
		$phone_section = [
			// 1391775,1592125, 1586584, 1891830,1590211, 1862181, 1377026, 1335798, 1368166,1348281, 1893083, 1582113,1365637, 1302068, 1505373,1337111, 1500025,1821717, 1390188, 1522150,
			1770511, 1836115, 1592197, 1832119, 1734976, 1366301, 1368179, 1590214, 1381641, 1521037, 1361164, 1500188, 1381643, 1590086,
		];
		foreach ($phone_section as $p) {
			self::combind_phone($p);
		}
	...
	{
		$phone_section = [
			1317663, 1382185, 1760269, 1303225, 1863090, 1351222, 1351225, 1862252, 1351226,
		];
		foreach ($phone_section as $p) {
			self::combind_phone($p);
		}
	...
	{
		$phone_section = [
			1391836, 1391844, 1500092, 1500045, 1502673, 1502131, 1502155, 1522134, 1580195, 1390146, 1582164, 1582196, 1358554, 1361161, 1364182, 1391749
		];
		foreach ($phone_section as $p) {
			self::combind_phone($p);
		}
	...
	{
		$phone_section = [
			1351228, 1351241, 1351243, 1351245, 1351281, 1351285, 1351286, 1351287
		];
		foreach ($phone_section as $p) {
			self::combind_phone($p);
		}
