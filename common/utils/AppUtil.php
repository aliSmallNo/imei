...
//				move_uploaded_file($tmp_name, $filePath);
	...
	{
		$arrays = [
			["array_​change_​key_​case", '将 array 数组中的所有键名改为全小写或大写。本函数不改变数字索引'],
			["array_​chunk", '将一个数组[平均]分割成多个数组。最后一个数组的单元数目可能会少于 size 个'],
			["array_​column", '返回input数组中键值为column_key的列， 如果指定了可选参数index_key，那么input数组中的这一列的值将作为返回数组中对应值的键'],
			["array_​combine", '返回一个数组，用一个数组的值作为其键名，另一个数组的值作为其值'],
			["array_​count_​values", '返回一个数组：数组的键是 array 里单元的值； 数组的值是 array 单元的值出现的次数'],
			["array_​diff_​assoc", '返回一个数组，该数组包括了所有在 array1 中但是不在任何其它参数数组中的值。注意和 array_diff() 不同的是键名也用于比较'],
			["array_​diff", '返回一个数组，该数组包括了所有在 array1 中但是不在任何其它参数数组中的值。注意键名保留不变。'],
			["array_diff_uassoc", '用用户提供的回调函数做索引检查来计算数组的差集'],
			["array_diff_ukey", '用回调函数对键名比较计算数组的差集'],
			["array_fill_keys", '使用指定的键和值填充数组'],
			["array_fill(s,n,v)", '用 v 参数的值将一个数组填充 n 个条目，键名由 s 参数指定的开始'],
			["array_filter(arr,cb)", '依次将 arr 数组中的每个值传递到 cb 函数。如果 cb 函数返回 true，则 arr 数组的当前值会被包含在返回的结果数组中。数组的键名保留不变。'],
			["array_flip(arr)", '返回一个反转后的 arr，例如 arr 中的键名变成了值，而 arr 中的值成了键名。'],
			["array_intersect_assoc(array1,array2,..) ", '返回一个数组，该数组包含了所有在 array1 中也同时出现在所有其它参数数组中的值'],
			["array_intersect(array1,array2,..)", '返回一个数组，该数组包含了所有在 array1 中也同时出现在所有其它参数数组中的值'],
			["array_intersect_key", '使用键名比较计算数组的交集'],
			["array_intersect_uassoc", '带索引检查计算数组的交集，用回调函数比较索引'],
			["array_intersect_ukey", '用回调函数比较键名来计算数组的交集'],
			["array_key_exists", '数组里有键 key 时，返回 TRUE。 key 可以是任何能作为数组索引的值'],
