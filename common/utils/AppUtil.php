...
class AppUtil
{
	...
	public static function createWxMenus()
	{
            ...
            "sub_button":[
                {
                    "type":"view",
                    "name":"马上订货",
                    "url":"https://wx.bpbhd.com/wx/market"
                },
                {
                    "type":"view",
                    "name":"我的订单",
                    "url":"https://wx.bpbhd.com/wx/order"
                },
                {
                    "type":"view",
                    "name":"积分兑换",
                    "url":"https://wx.bpbhd.com/wx/rewards"
                },
                {
                    "type":"view",
                    "name":"注册/登录",
                    "url":"https://wx.bpbhd.com/wx/login"
                }
            ]
        },
        {
            "type":"view",
            "name":"一元夺宝",
            "url":"https://wx.bpbhd.com/one/home"
        },
        {
            "name":"更多服务",
            "sub_button":[
            	{
            		"type":"view",
            		"name":"诚邀加盟",
            		"url":"http://h.eqxiu.com/s/sXTDMsW3?eqrcode=1&from=timeline&isappinstalled=0"
        		},
                {
                    "type":"view",
                    "name":"我是供应商",
                    "url":"https://wx.bpbhd.com/wx/shop"
                },
                {
                    "type":"view",
                    "name":"我是司机",
                    "url":"https://wx.bpbhd.com/wx/driver"
                },
                {
                    "type":"view",
                    "name":"安卓司机版",
                    "url":"http://fir.im/bpdjsj"
                }
            ]
        }
    ]
...
EOT;
		...
		$res = self::postJSON($url, $postData);
		return $res;
	}
		...
		$postData = [
			"button" => [
				[
					...
					"sub_button" => [
						[
							"type" => "view",
							"name" => "马上订货",
							"url" => "https://wx.bpbhd.com/wx/mall#fhome"
						],
						[
							"type" => "view",
							"name" => "我的订单",
							"url" => "https://wx.bpbhd.com/wx/mall#forder"
						],
						[
							"type" => "view",
							"name" => "注册/登录",
				...
				[
					"type" => "view",
					"name" => "奔跑团",
					"url" => "https://wx.bpbhd.com/wx/trade"
				],
				[
					"name" => "更多服务",
					"sub_button" => [
						...
						[
							"type" => "view",
							"name" => "广告业务",
							"url" => "https://wx.bpbhd.com/wx/adpages"
						],
						[
							"type" => "view",
							"name" => "我是供应商",
							"url" => "https://wx.bpbhd.com/wx/vendor"
						],
						[
							"type" => "view",
							"name" => "我是司机",
							"url" => "https://wx.bpbhd.com/wx/delivery"
						],
						[
							"type" => "view",
							"name" => "我是销售",
							"url" => "https://wx.bpbhd.com/wx/sale"
						]
					]
				]
			]
	...
	public static function getWeather($cityName, $num = 3)
	{
		...
		//$url = "http://apis.baidu.com/heweather/weather/free?city=" . urlencode($cityName);
		$url = "https://api.thinkpage.cn/v3/weather/daily.json?key=c8ljlrztptwrm16i&location=" . urlencode($cityName) . "&language=zh-Hans&unit=c&start=0&days=" . $num;
		$ret = self::httpGet2($url, []);
		$ret = json_decode($ret, true);

		if (isset($ret['results'])) {
			//$ret = array_values($ret);
			$daily_forecast = $ret['results'][0]['daily'];
			$city = $ret['results'][0]['location']['name'];
			$weatherInfo = [];
			if ($daily_forecast) {
				foreach ($daily_forecast as $k => $v) {
					$cond_day = $v['text_day']; //白天天气
					$code_day = $v['code_day']; //白天天气图片
					$cond_night = $v['text_night']; //晚上天气
					$code_night = $v['code_night']; //晚上天气图片
					list($iconUrl, $bgUrl) = self::weatherImage($cond_day, $code_day);
					$dayName = str_replace('-', '月', substr($v['date'], 5)) . '日';
					if ($k == 0) {
						$dayName = "今天";
					}
					$weatherInfo[] = [
						...
						"pic" => $iconUrl,
						"bgurl" => $bgUrl,
						//"qing" => $cond_day . '转' . $cond_night,
						"cond_day" => $cond_day,
						'cond_night' => $cond_night,
						'temp' => $v['low'] . '~' . $v['high'] . '℃',
						'wind' => $v['wind_direction'] . '风' . $v['wind_scale'] . '级',
						//'pm25' => 'PM2.5:' . $v['hum'],
						'date' => $dayName
					];
				}

				$redis->set($redisKey, json_encode($weatherInfo));
