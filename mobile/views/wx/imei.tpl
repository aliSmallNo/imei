<section id="fsms">
	<div class="form single">
		<input type="number" placeholder="请输入您的手机号码" class="input-s">
		<div class="flex">
			<input type="number" placeholder="请输入验证码" class="input-s">
			<a href="javascript:;" class="btn-s s1">发送验证码</a>
		</div>
		<p class="help-block">
			<a href="javascript:;" class="change">点击这里切换成「媒婆」身份</a>
		</p>
		<a href="javascript:;" class="m-submit">登录</a>
	</div>
</section>
<section id="frole">
	<h1><img src="/images/logo62.png" alt="" class="logo">欢迎加入「微媒」</h1>
	<div class="roles">
		<div class="title">请选择您要注册的身份</div>
		<a href="/wx/mreg" class="btn" data-tag="matcher">当媒婆，我推荐单身</a>
		<a href="/wx/sreg" class="btn on" data-tag="single">我单身，给自己找对象</a>
	</div>
	<a href="javascript:;" class="m-next">进入单身注册</a>
</section>
<script type="text/template" id="tpl_wx_info">
	{{$wxInfoString}}
</script>
<script data-main="/js/imei.js?v=1.1.1" src="/assets/js/require.js"></script>